# NCTech Capture Workflow

## Scope

This document defines the supported NCTech ingestion path in Byakugan and the
...
- The loader is implemented in `src/byakugan_app/io/nctech_capture.py`.
- Parser behavior is covered by `tests/test_nctech_capture.py`.
...
1. Add robust residual-quality thresholds and uncertainty reporting for
   two-frame triangulation.
2. Add sequence-aware triangulation mode with persistent tracklets
   (multi-frame feature intersection, not only pairwise).
3. Add raw 4-sensor ingestion path from calibration + timestamps + videos.
