...
from byakugan_app.io.capture_sequence import discover_stitched_capture
...
def test_discover_stitched_capture_enriches_frame_with_gps_accuracy(tmp_path: Path):
    ...
    (output / "capture-gps.txt").write_text(
        "systemtime_sec,systemtime_usec,hMSL,hAcc,vAcc,headAcc\n"
        "100,250000,42123,850,1200,25000\n",
        encoding="utf-8",
    )

    capture = discover_stitched_capture(root)
    frame = capture.frames[0]
    assert frame.horizontal_accuracy_m == pytest.approx(0.85)
    assert frame.vertical_accuracy_m == pytest.approx(1.2)
    assert frame.heading_accuracy_deg == pytest.approx(0.25)
    assert frame.altitude_msl_m == pytest.approx(42.123)
